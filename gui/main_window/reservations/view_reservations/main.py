...
class ViewReservations(Frame):
    ...
    def handle_refresh(self):
        ...
        reservations = db_controller.get_reservations()
        if reservations:  # Check if data is retrieved successfully
            # Update self.reservation_data with the fetched data
            self.reservation_data = reservations
            # Insert the data into the Treeview
            for row in self.reservation_data:
                self.treeview.insert("", "end", values=row)
        else:
            # Use parent's reservation data as fallback if no data is retrieved
            self.reservation_data = self.parent.reservation_data
            if self.reservation_data:  # Check if parent's reservation data is available
                # Insert the fallback data into the Treeview
                for row in self.reservation_data:
                    self.treeview.insert("", "end", values=row)
            else:
                # If no data is available, print a message or handle it accordingly
