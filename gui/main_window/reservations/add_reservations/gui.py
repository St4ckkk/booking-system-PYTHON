...
class AddReservations(Frame):
    ...
    def save(self):
        ...
        g_id = self.data["g_id"].get()
        meal = self.data["meal"].get()
        r_id = self.data["r_id"].get()
        check_in = self.data["check_in"].get()

        if db_controller.is_double_booking(r_id, check_in):
            messagebox.showerror(
                "Error", "Double booking detected. Please choose a different time or room."
            )
            return

        # Save the reservation
        result = db_controller.add_reservation(g_id, meal, r_id, check_in)
